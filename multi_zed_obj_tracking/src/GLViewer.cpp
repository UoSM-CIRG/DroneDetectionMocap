...
void GLViewer::addSKeleton(sl::BodyData &obj, Simple3DObject &simpleObj, sl::float3 clr_id, bool raw,  sl::BODY_FORMAT format) {
    switch (format)
    {
...
void GLViewer::updateBodies(sl::Bodies &bodies, std::map<sl::CameraIdentifier, sl::Bodies>& singldata, sl::FusionMetrics& metrics) {
    ...
    if (bodies.is_new) {
        ...
        for(auto &it:bodies.body_list) {
            ...
            if (colors_sk.find(it.id) == colors_sk.end()) {
                float hh = uint_dist360(rng) / 60.f;
                colors_sk[it.id] = newColor(hh);
            }

            if (renderBody(it, bodies.is_tracked))
                addSKeleton(it, skeletons, colors_sk[it.id], false, bodies.body_format);
        }
    }
